...
do {\
	testCounter++;\
	if (!(cond)) {\
		success = false; failedCount++; os << testCounter << ": Fail [" << __FILE__ << ", " << __LINE__ << "]" << endl;\
	}\
}while(false)
...
do {\
	testCounter++;\
	if (!(cond)) {\
		success = false; failedCount++; os << testCounter << ": Fail: " << message << " [" << __FILE__ << ", " << __LINE__ << "]" << endl << endl;\
	}\
}while(false)

#define TESTE(code,exception,message) \
do {\
	testCounter++;\
	try {\
		code;\
		success = false; failedCount++; os << testCounter << ": Failed to throw exception: " << message << endl << "\t[" << __FILE__ << ", " << __LINE__ << "]" << endl << endl;\
	}catch(exception e){}\
}while(false)
